"""Patch cc_torch: keep the cell's TORCH_CUDA_ARCH_LIST authoritative on aarch64.
...
Upstream get_extensions() does, verbatim:
...
compiles connected_components.cu for its default architecture alone -- sm_75
under CUDA 13 -- and the artifact fails verify_conda's SASS census
("want ['80','87','90','100','110','120'], got ['75']", run 34729955300).

On linux-64 CC is "x86_64-conda-linux-gnu-cc" (no "arch" substring), which is
why every linux-64 cc-torch artifact built correctly with the full arch list.
...
compiles for the cell's TORCH_CUDA_ARCH_LIST, exactly as the peer torch-scatter
(which never appended -ccbin) already does on both platforms.
"""
import pathlib
import sys
...
if MARKER in text:
    print("cc_torch patch: already applied")
    sys.exit(0)

require(
    text.count(OLD) == 1,
    "cc_torch: the -ccbin={CC} append block matched "
...
require(
    ...
    "cc_torch: the -ccbin append still survives in setup.py on disk",
)
import ast  # noqa: E402

ast.parse(final)
print("cc_torch patch: -ccbin={CC} append removed; TORCH_CUDA_ARCH_LIST authoritative")
