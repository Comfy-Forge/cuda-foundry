...
shopt -s nullglob
CUW_WHEELS=("$CUW_WHEELHOUSE"/*.whl)
shopt -u nullglob
if [ "${#CUW_WHEELS[@]}" -ne 1 ]; then
  echo "::error::expected exactly 1 wheel in $CUW_WHEELHOUSE, found ${#CUW_WHEELS[@]}: ${CUW_WHEELS[*]:-none}. The .conda and the published wheel must come from ONE file; anything else means the compile produced nothing or the wheelhouse was not cleared." >&2
  exit 1
fi
CUW_WHEEL="${CUW_WHEELS[0]}"
